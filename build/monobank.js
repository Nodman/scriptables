...
class Monobank {
  constructor(parentScriptName) {
    this.parentScriptName = parentScriptName;
    this.readSettings();
  }

  static getTodaysExpanses({
    currentPeriod
  }) {
  ...
  static getStatsForPeriod({
    ...
    while (monthlyHistory.length < period && yeatsAvailable.length) {
      ...
      Object.keys(yearPeriod).map(Number).sort((a, b) => b - a).slice(-period + monthlyHistory.length).forEach(month => {
  ...
  async getAccountId(parentScriptName) {
  ...
  fetchData(path) {
    if (this.isTokenProvided()) {
      ...
      return fetchJson(url, this.requestHeaders);
    }
  ...
  async getAccountCacheKey(parentScriptName) {
  ...
  async setupToken() {
    ...
    const setupAlert = createAlert('Monobank API token is required', `In order to fetch data from Monobank you need to provide your generated API token.
If you don't have one - please visit https://api.monobank.ua and follow the instructions.`, 'Cancel');
  ...
  async setupFilters(parentScriptName) {
    ...
    const settings = (await this.settings) ?? {};
    const setupFiltersAlert = createAlert('Set statement items filter', 'Enter semicolon separated values, e.g. "rent;netflix;spotify". Items with one of those words in description will be excluded from accounting', 'Cancel');
  ...
  async setupAccount(parentScriptName) {
    const scriptName = parentScriptName ?? this.parentScriptName;
    const setupAccountsAlert = createAlert('Select account', 'ID of selected account will be copied to clipboard and used for this script.', 'Cancel');
  ...
  async fetchAccounts() {
    const response = await this.fetchData(API_PATH.PERSONAL_INFO);
  ...
  async fetchAccountStatement(fromArg, toArg) {
  ...
  async fetchLatestStatement() {
    ...
    const {
      currentPeriod,
      history = {}
    } = (await cache.read(accountCacheKey)) ?? {};
    ...
    for (let index = response.length - 1; index >= 0; index--) {
      ...
      if (filter(item)) {
        ...
        currentPeriod.statement[date] = currentPeriod.statement[date] ?? []; // @ts-ignore

        currentPeriod.statement[date].push({
          id,
          time,
          amount,
          description,
  ...
  async editOP({
    ...
    const statement = await cache.read(accountCacheKey);
  ...
  async editStatementItem(parentScriptName) {
    const accountCacheKey = await this.getAccountCacheKey(parentScriptName);
    const {
      currentPeriod: {
        statement
    ...
    switch (valueActionIndex) {
      ...
      case 0:
        amount = Number(valueAlert.textFieldValue(0)) * 1e2;
        break;

      case 1:
        amount = originalAmount ? null : -1 * currentAmount;
        break;
    }
